...
bool ServiceProvider::validateMeasurementList(string received_list) {
    uint8_t *content;
    int content_size;
    ...
    if (content_size != -1) {
        string compare_file = ByteArrayToNoHexString(content, content_size);

        if (received_list.compare(compare_file) == 0) {
            Log("Measurement list is valid");
            return true;
        } else {
            Log("Measurement list is not valid!");
            return false;
        }
    } else
        Log("Error reading compare file", log::error);

    return false;
}


int ServiceProvider::decryptMessage(Messages::SecretMessage ml_msg, uint8_t **decrypted) {
    int ret = 0;
    int secret_size = ml_msg.size();
    uint8_t gcm_mac[16];
    ...
    *decrypted = (uint8_t*) malloc(sizeof(uint8_t) * secret_size);
    ...
    for (int i=0; i<secret_size; i++)
        secret[i] = ml_msg.encrypted_content(i);
    ...
    if (SGX_SUCCESS != ret) {
        Log("Error, decryption failed", log::error);
        return -1;
    }
...
int ServiceProvider::encryptMessage(uint8_t *source, int size_source, uint8_t *encrypted, uint8_t *gcm_mac) {
    ...
    if (SGX_SUCCESS != ret) {
        Log("Error, encryption failed", log::error);
        return -1;
    }

    memcpy(gcm_mac, mac, 16);

    return 0;
}


int ServiceProvider::sp_ra_proc_measurement_list(Messages::SecretMessage sec_msg, Messages::SecretMessage *new_msg) {
    int error = 0;

    uint8_t *decrypted;
    int secret_size = sec_msg.size();

    error = this->decryptMessage(sec_msg, &decrypted);

    if (!error) {
        uint8_t *decrypted_msl = (uint8_t*) malloc(sizeof(uint8_t) * (secret_size));
        memset(decrypted_msl, '\0', (secret_size));

        memcpy(decrypted_msl, decrypted, (secret_size));

        bool valid = this->validateMeasurementList(ByteArrayToNoHexString(decrypted_msl, (secret_size)));

        memset(validation_result, '\0', MAX_VERIFICATION_RESULT);
        validation_result[0] = 1;	//indicates measurement list result

        string str_hmac_nonce;
        uint8_t *hmac_key, *hmac_nonce;
        int len_hmac_key = 0, len_hmac_nonce = 0;

        if (valid) {
            validation_result[1] = 1;
            this->hmac = CalculateHMAC(&str_hmac_key, &str_hmac_nonce);

            len_hmac_key = StringToByteArray(str_hmac_key, &hmac_key);
            len_hmac_nonce = StringToByteArray(str_hmac_nonce, &hmac_nonce);
        } else {
            validation_result[1] = 0;
        }

        int total_len = (MAX_VERIFICATION_RESULT + HMAC_KEY_LENGTH + HMAC_NONCE_LENGTH);
        uint8_t encrypted[total_len];

        uint8_t total_data[total_len];
        memcpy(total_data, validation_result, MAX_VERIFICATION_RESULT);
        memcpy(total_data+MAX_VERIFICATION_RESULT, hmac_key, HMAC_KEY_LENGTH);
        memcpy(total_data+MAX_VERIFICATION_RESULT+HMAC_KEY_LENGTH, hmac_nonce, HMAC_NONCE_LENGTH);

        uint8_t gcm_mac[16];
        int ret = 0;
        ret = this->encryptMessage(total_data, total_len, encrypted, gcm_mac);

        if (SGX_SUCCESS == ret) {
            Log("Encrypted measurement result");

            new_msg->set_size(total_len);

            for (int i=0; i<total_len; i++)
                new_msg->add_encrypted_content(encrypted[i]);

            for (int i=0; i<16; i++)
                new_msg->add_mac_smk(gcm_mac[i]);
        }
    }
...
int ServiceProvider::sp_ra_proc_app_att_hmac(Messages::SecretMessage *new_msg, string hmac_key, string filename) {
    string str_nonce;
    uint8_t *hmac_full;
    int len_hmac_full;

    this->hmac = CalculateHMAC(&hmac_key, &str_nonce);

    string hmac_string = filename + str_nonce;
    len_hmac_full = StringToByteArray(hmac_string, &hmac_full);

    uint8_t gcm_mac_hmac[16];
    uint8_t *encrypted_hmac = (uint8_t*) malloc(sizeof(uint8_t) * len_hmac_full);
    int ret = this->encryptMessage(hmac_full, len_hmac_full, encrypted_hmac, gcm_mac_hmac);

    if (SGX_SUCCESS == ret) {
        Log("Encrypted nonce result");

        new_msg->set_size(len_hmac_full);

        for (int i=0; i<len_hmac_full; i++)
            new_msg->add_encrypted_content(encrypted_hmac[i]);

        for (int i=0; i<16; i++)
            new_msg->add_mac_smk(gcm_mac_hmac[i]);
    }
...
int ServiceProvider::sp_ra_proc_ra_hmac(Messages::SecretMessage sec_msg) {
    int error = 0;
    ...
    if (!error) {
        uint8_t filename[FILE_UUID_LENGTH];
        uint8_t ra_hmac[HMAC_LENGTH];

        memcpy(filename, decrypted, FILE_UUID_LENGTH);
        memcpy(ra_hmac, decrypted+FILE_UUID_LENGTH, HMAC_LENGTH);

        this->hmac_key_filename = ByteArrayToNoHexString(filename, FILE_UUID_LENGTH);
        string str_ra_hmac = ByteArrayToString(ra_hmac, HMAC_LENGTH);

        if (hmac.compare(str_ra_hmac) != 0) {
            Log("HMAC and RA-HMAC are not identical!");
            this->success = false;
        } else {
            Log("HMAC nad RA-HMAC are identical");
            this->success = true;
        }
    } else {
        this->success = false;
...
int ServiceProvider::sp_ra_proc_app_hmac(Messages::SecretMessage sec_msg) {
    ...
    if (!error) {
        string app_hmac = ByteArrayToString(decrypted, HMAC_LENGTH);

        if (this->hmac.compare(app_hmac) != 0) {
            Log("HMAC and App-HMAC are not identical!");
            this->success = false;
        } else {
            Log("HMAC and App-HMAC are identical!");
            this->success = true;
        }
    }
...
int ServiceProvider::sp_ra_app_hmac_resp(Messages::SecretMessage *new_msg,
        ...
        int x509_crt_size) {
    memset(validation_result, '\0', MAX_VERIFICATION_RESULT);
    validation_result[0] = 2;	//indicates app hmac result
    ...
    uint8_t encrypted[MAX_VERIFICATION_RESULT];
    uint8_t gcm_mac[16];
    ...
    if (SGX_SUCCESS == ret) {
        Log("Encrypt result message");

        new_msg->set_size(MAX_VERIFICATION_RESULT);
