...
uint32_t MessageHandler::getExtendedEPID_GID() {
    ...
    if (SGX_SUCCESS != ret) {
        ret = -1;
        Log("Error, call sgx_get_extended_epid_group_id fail", log::error);
        return ret;
    }
...
string MessageHandler::handleMSG2(Messages::MessageMSG2 msg) {
    ...
    if (SGX_SUCCESS != (sgx_status_t)ret) {
        Log("Error, call sgx_ra_proc_msg2 fail, error code: 0x%x", ret, log::error);
    } else {
...
vector<string> MessageHandler::incomingHandler(string v, int type) {
    ...
    case RA_ATT_RESULT: {
        ...
        if (ret && att_msg.type() == RA_ATT_RESULT) {
            ...
            res.push_back(to_string(RA_MEASUREMENT));
        }
    }
    break;
    case RA_MEASUREMENT_RES: {
        Messages::SecretMessage sec_msg;
        ret = sec_msg.ParseFromString(v);
        if (ret && (sec_msg.type() == RA_MEASUREMENT_RES)) {
            s = this->handleMeasurementListResult(sec_msg);
    ...
    case RA_HMAC: {
        Messages::SecretMessage sec_msg;
        ret = sec_msg.ParseFromString(v);
        if (ret && (sec_msg.type() == RA_HMAC)) {
            s = this->handleApplicationNonce(sec_msg);
            res.push_back(to_string(APP_HMAC));
        }
    }
    ...
    case APP_HMAC_RES: {
        Messages::SecretMessage sec_msg;
        ret = sec_msg.ParseFromString(v);
        if (ret && (sec_msg.type() == APP_HMAC_RES)) {
            this->handleAppHMACResult(sec_msg);
        }
    }
    break;
    default:
