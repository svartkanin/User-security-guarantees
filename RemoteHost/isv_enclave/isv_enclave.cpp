            ...
            if (decrypted[0] == 0) {
                ...
                if (decrypted[1] != 1) {
                    ...
                    if (secret_size > max_verification_length) {
                        ...
                        memset(hmac, '\0', HMAC_LENGTH);	//defined in Abstract_ISV_Enclave.h

                        int error = calculateHMAC(key, nonce, hmac);
...
sgx_status_t encrypt_hmac(sgx_ra_context_t context,
                          uint8_t *filename,
    ...
    do {
        ...
        int fullsize = (HMAC_LENGTH + filename_size);
        uint8_t *to_encrypt = (uint8_t*) malloc(sizeof(uint8_t) * fullsize);
        memcpy(to_encrypt, filename, filename_size);
        memcpy(to_encrypt+filename_size, hmac, HMAC_LENGTH);
